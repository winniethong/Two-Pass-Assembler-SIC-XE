...
int startAddress,locctr = 0,length, baseReg = 0;// Chloe Kershner - 826281307
// Winnie Thong   -
#include <iostream>
#include <fstream>
#include <cstring>
        ...
        for (int i = 0; i < SymbolTable.size(); ++i) {
            if (std::strcmp(SymbolTable[i].symbol, operand3) == 0) { // if found
                ...
                if (bFlag == -1) { // if the b flag has not yet been set,
                    // set b flag depending on range PC + disp falls in
                    int locctrDecimal = locctr;
                    char locctrHex[4];
                    snprintf(locctrHex, sizeof(locctrHex), "%03X", locctrDecimal);
                    int num = symb.addr + locctr;
                    // calculate if disp + PC is greater than 2047
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        fclose(intermediateFile);
        fclose(file);
        SymbolTable.clear(); // clear Symbol Table and literal table
        LiteralTable.clear();
